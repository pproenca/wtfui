...
def test_import_hook_can_be_installed():
    ...
    try:
        ...
        assert any(isinstance(f, WtfUIImportHook) for f in sys.meta_path)
    finally:
        uninstall_import_hook()
...
def test_import_hook_caches_transformations():
    ...
    install_import_hook()
    try:
        hook = next(f for f in sys.meta_path if isinstance(f, WtfUIImportHook))
...
def test_import_hook_respects_debug_env():
    """Import hook respects WTFUI_DEBUG environment variable."""
    original = os.environ.get("WTFUI_DEBUG")
    try:
        os.environ["WTFUI_DEBUG"] = "1"
        hook = WtfUIImportHook()
        assert hook._debug_mode is True
    finally:
        if original is None:
            os.environ.pop("WTFUI_DEBUG", None)
        else:
            os.environ["WTFUI_DEBUG"] = original
...
def test_import_hook_ignores_non_client_modules():
    ...
    try:
        ...
        spec = hook.find_spec("wtfui.signal", None)
        assert spec is None
