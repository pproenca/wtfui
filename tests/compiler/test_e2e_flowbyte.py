...
including all features implemented in the FlowByte v2 architecture.
"""
...
"""
    ...
    assert binary.endswith(b"\xff")  # HALT
...
"""
    ...
    # Strings in pool
    assert b"background" in binary
    assert b"blue" in binary
    assert b"padding" in binary
...
"""
    # Single-threaded
    binary_single = compile_to_flowbyte(source)
...
"""
    ...
    # Header: "FLOW" (4 bytes) + version (2 bytes)
    assert binary[0:4] == b"FLOW"
