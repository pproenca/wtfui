"""Pipeline integration tests for FlowByte compiler.

Tests the complete compilation pipeline:
    Source → AST → Graph → Analyzer → Validator → Compiler → .fbc
...
from flow.compiler.evaluator import DYNAMIC_STYLE, safe_eval_style
from flow.compiler.validator import BoundarySentinel
...
count = Signal(0)
"""
    bytecode = compile_to_flowbyte(source)

    assert bytecode.startswith(b"FLOW")
...
def test_cache_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Compile
        bytecode = compile_to_flowbyte(source_file.read_text())
...
def test_pipeline_cache_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cache_dir = Path(tmpdir) / ".flowcache"
