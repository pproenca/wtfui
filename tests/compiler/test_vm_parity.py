...
def _extract_opcodes_from_ts() -> set[int]:
    ...
    vm_ts = Path(__file__).parent.parent.parent / "src" / "flow" / "static" / "vm.ts"
    if not vm_ts.exists():
        return set()
...
def _extract_opcodes_from_embedded() -> set[int]:
    ...
    #              0x40-0x42 (control), 0x60-0x69 (dom), 0x90 (rpc),
    #              0xA0-0xA5 (stack), 0xC0 (intrinsic), 0xFF (halt)
    valid_opcode_ranges = [
        (0x01, 0x03),  # Signals
...
def test_embedded_vm_has_all_python_opcodes():
    ...
    # Remove legacy register-based ADD/SUB (0x20, 0x21) which are deprecated
    legacy_deprecated = {0x20, 0x21}
    # RPC_CALL (0x90) is intentionally not in embedded VM - it's minimal fallback
    intentionally_missing = {OpCode.RPC_CALL}
...
def test_embedded_vm_missing_rpc_call():
    ...
    assert (
        OpCode.RPC_CALL not in embedded_opcodes
