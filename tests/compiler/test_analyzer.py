...
def test_get_client_modules():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        (root / "counter.py").write_text("from flow.signal import Signal")
        (root / "button.py").write_text("from flow.ui import Button")
...
def test_get_server_modules():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (root / "ui.py").write_text("from flow.ui import Div")
