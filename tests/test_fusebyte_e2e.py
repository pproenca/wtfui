...
class TestFullPipeline:
    ...
    def test_counter_app_compiles(self) -> None:
        """Full counter app compiles to valid WtfUIByte."""
        source = """
count = Signal(0)
...
class TestBinaryFormat:
    ...
    def test_header_version(self) -> None:
        ...
        binary = compile_to_wtfuibyte("x = Signal(0)")
...
"""
        ...
        result = benchmark(lambda: compile_to_wtfuibyte(source))

        assert len(result) > 0

        avg_time_ms = benchmark.stats.stats.mean * 1000
        print(f"\n[WtfUIByte E2E] Compilation time: {avg_time_ms:.4f} ms")
